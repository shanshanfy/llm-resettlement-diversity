...
def _bootstrap_with_chooser(choose_fn, n_bootstrap, seed):
    ...
    for b in range(n_bootstrap):
        ...
        pop_b = {k: v.copy() if hasattr(v, 'copy') else v for k, v in pop.items()}
...
def bootstrap_random_uniform(n_bootstrap=100, seed=42):
    ...
    return _bootstrap_with_chooser(lambda i, rng, pop, ab: int(rng.choice(N_SITES, p=p)), n_bootstrap, seed)
...
def bootstrap_llm_targets(decisions, n_bootstrap=100, seed=42):
    ...
    for d in decisions:
        ...
        if site is None:
            continue
        hh = d['hh_idx']
        key = (int(pop['role'][hh]), int(pop['ethnicity'][hh]), int(asset_bin[hh]))
        stratum_choices.setdefault(key, []).append(site)
    global_choices = []
    for choices in stratum_choices.values():
        global_choices.extend(choices)
    global_dist = np.bincount(global_choices, minlength=N_SITES) / max(len(global_choices), 1)
    target_names = list(OBSERVED_TARGETS.keys())
    n_targets = len(target_names)
    logliks = np.zeros((n_bootstrap, n_targets))
    for b in range(n_bootstrap):
        site_choice = -np.ones(pop['role'].shape[0], dtype=int)
        for i in np.where(pop['survived'])[0]:
            key = (int(pop['role'][i]), int(pop['ethnicity'][i]), int(asset_bin[i]))
            if key in stratum_choices and len(stratum_choices[key]) > 0:
                site_choice[i] = int(rng.choice(stratum_choices[key]))
            else:
                p = global_dist if global_dist.sum() > 0 else np.full(N_SITES, 1 / N_SITES)
...
def main(llm_paths, out_path, n_bootstrap=100):
    ...
    for fam in ['pi1', 'pi2', 'pi3']:
        ll = compute_per_target_logliks(posteriors[fam]['particles'], fam, n_subsample=150, seed=42)
        e, s, et = psis_loo_elpd(ll)
        elpd[fam] = e
        se[fam] = s
        elpd_t[fam] = et
    ...
    for path in llm_paths:
        ...
        se[label] = s
        elpd_t[label] = et
        panel[label] = {'model': data['model'], 'n_calls_made': data['n_calls_made'], 'n_unparseable': data['n_unparseable'], 'occupancy': data['aggregated_targets']['three_site_occupancy'], 'path': str(path)}
        print(f'  {label:>14} ELPD = {e:>8.2f} (SE {s:>5.2f})')
    families = ['random', 'marginal', 'logit', 'pi1', 'pi2', 'pi3'] + list(panel.keys())
    ...
    for t, name in enumerate(target_names):
        ...
        print(row)
    results = {'elpd': {k: float(v) for k, v in elpd.items()}, 'se': {k: float(v) for k, v in se.items()}, 'elpd_t': {k: list(map(float, v)) for k, v in elpd_t.items()}, 'target_names': target_names, 'panel': panel, 'families': families, 'observed_three_site_occupancy': list(OBSERVED_TARGETS['three_site_occupancy'])}
    with open(out_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f'\nSaved {len(families)}-way analysis (with baselines) to {out_path}')
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--llm', action='append', required=True, help='Path to llm_baseline_*.json. May repeat.')
